...
use crate::{core::{Config, Problem}, input::{Input, Uncertainties}};
...
impl<'a, E, R, M, Va, Co> Builder<'a, E, R, M, Unset, Va, Co> {
    pub fn with_input_expectations(self, expectation_values: ArrayView1<'a, E>) -> Builder<'_, E, R, M, Set, Va, Co> {
        Builder {
            rng: self.rng,
            model: self.model,
            config: self.config,
...
impl<'a, E, R, M, Ex> Builder<'a, E, R, M, Ex, Unset, Unset> {
    pub fn with_input_variances(self, variances: ArrayView1<'a, E>) -> Builder<'_, E, R, M, Ex, Set, Unset> {
        Builder {
            rng: self.rng,
            model: self.model,
            config: self.config,
...
impl<'a, E, R, M, Ex> Builder<'a, E, R, M, Ex, Unset, Unset> {
    fn with_input_covariances(self, covariances: ArrayView2<'a, E>) -> Builder<'_, E, R, M, Ex, Unset, Set> {
        Builder {
            rng: self.rng,
            model: self.model,
            config: self.config,
...
impl<'a, E: Float + ToPrimitive, R: Rng, P> Builder<'a, E, R, P, Set, Set, Unset> {
    pub fn build(self) -> Problem<'a, E, R, P> {
        ...
        let number_of_trials = 10_000.max(1 / (1 - 100 * config.required_coverage_probability.to_usize().unwrap()));
        Problem {
            config,
            number_of_trials,
            rng: self.rng,
...
#[cfg(test)]
mod test {
    use crate::{Model, builder::Builder, core::compute_tolerance};

    use rand_isaac::Isaac64Rng;
    use ndarray::Array1;
    use ndarray_rand::{rand::{Rng, SeedableRng}, rand_distr::{Normal, Distribution}};
    ...
    fn linear_model_has_expected_properties() {
        ...
        impl Model<f64, Isaac64Rng> for TestModel {
            fn apply(&self, inputs: ndarray::Array1<f64>, rng: &mut Isaac64Rng) -> std::result::Result<ndarray::Array1<f64>, Box<dyn std::error::Error>> {
                let dists = self.means.iter().zip(self.var)
                    .map(|(&mean, var)| Normal::new(mean, var.sqrt()))
                    .collect::<Result<Vec<_>, _>>()?;

                let res = inputs.into_iter()
        ...
        let expectation_values = (0..num_data_points)
            .map(|_| rng.gen())
            .collect::<Array1<f64>>();
        let variances = expectation_values.iter().map(|x| x / 100.0).collect::<Array1<f64>>();
