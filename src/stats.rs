...
mod test {
    ...
    fn epoch_expectation_matches_manual_compute() {
        ...
        for _ in 0..num_samples {
            let row = values.sample_iter(&mut rng).take(row_size).collect();
            epochdata.add_row(row).unwrap();
        }
    ...
    fn epoch_expectation_roughly_matches_expected_when_all_equal() {
        ...
        for computed in stats.expectation {
    ...
    fn epoch_standard_deviation_roughly_matches_expected_when_all_equal() {
        ...
        for computed in stats.covariance.diag().mapv(|var| var.sqrt()) {
